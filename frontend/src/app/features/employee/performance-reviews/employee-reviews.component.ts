      ...
      <mat-tab-group>
        ...
        <mat-tab label="My Reviews">
          <div style="margin-top:16px">
            ...
            <!-- Active Reviews -->
            <div *ngFor="let r of activeReviews" class="card"
              ...
              <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-wrap:wrap;gap:10px">
                <div>
                  <div style="font-weight:700;color:#1a1a2e;font-size:15px">
                    Review — {{ r.createdAt | date:'dd MMM yyyy' }}
                  </div>
                  <div style="font-size:12px;color:#aaa;margin-top:2px">ID #{{ r.id }}</div>
                </div>
              ...
              <div *ngIf="r.managerFeedback"
                ...
                <div style="font-size:14px;color:#333;margin-bottom:6px">{{ r.managerFeedback.feedback }}</div>
                <div style="font-size:12px;color:#888">
                  Manager Rating: <strong style="color:#1565c0">{{ r.managerFeedback.managerRating }}/5 ⭐</strong>
                </div>
              </div>
              ...
              <div *ngIf="showHistory">
                <div *ngFor="let r of historyReviews" class="card"
                  ...
                  <div *ngIf="r.managerFeedback"
                    ...
                    <div style="font-size:12px;color:#555">{{ r.managerFeedback.feedback }}</div>
                    <div style="font-size:11px;color:#888;margin-top:2px">
                      Rating: {{ r.managerFeedback.managerRating }}/5
                    </div>
                  </div>
                </div>
              </div>
...
export class EmployeeReviewsComponent implements OnInit {
  ...
  activeReviews: PerformanceReview[] = [];
  ...
  loadReviews(): void {
    this.loading = true;
    this.employeeService.getPerformanceReviews().subscribe({
      next: (d: any) => {
        this.allReviews = Array.isArray(d) ? d : (d?.data || []);
        this.activeReviews = this.allReviews.filter(r =>
          r.status === 'SUBMITTED' || r.status === 'UNDER_REVIEW' || r.status === 'DRAFT'
        );
  ...
  submitReview(): void {
    ...
    this.employeeService.createPerformanceReview(this.reviewForm.value).subscribe({
      next: () => {
  ...
  getStatusClass(s: string): string {
    const m: Record<string,string> = {
      ...
      'UNDER_REVIEW': 'PENDING', 'DRAFT': 'REJECTED'
    };
    return m[s] || 'PENDING';
  }
}