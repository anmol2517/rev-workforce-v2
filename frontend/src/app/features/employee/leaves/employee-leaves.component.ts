  ...
  template: `
    <div class="page-container">
      ...
      <mat-tab-group>
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px">
              <div *ngFor="let lb of leaveBalance" class="card" style="padding:20px;border-left:4px solid #4a148c">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px">
                  <span style="font-size:17px;font-weight:700;color:#1a1a2e">{{ lb.leaveType }}</span>
                  <span>
                    <span style="font-size:26px;font-weight:800;color:#4a148c">{{ lb.remaining }}</span>
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px">
                  <div style="padding:10px;background:#f8f9fa;border-radius:8px;text-align:center">
                    ...
                    <div style="font-weight:700;color:#333;font-size:16px">{{ lb.total }}</div>
                  </div>
                  <div style="padding:10px;background:#fce4ec;border-radius:8px;text-align:center">
                    ...
                    <div style="font-weight:700;color:#c62828;font-size:16px">{{ lb.used }}</div>
                  </div>
                  <div style="padding:10px;background:#e8f5e9;border-radius:8px;text-align:center">
                    ...
                    <div style="font-weight:700;color:#2e7d32;font-size:16px">{{ lb.remaining }}</div>
                  </div>
                </div>
                ...
                <div style="display:flex;justify-content:space-between;margin-top:4px;font-size:11px;color:#aaa">
                  <span>0</span><span>{{ getUsedPct(lb) }}% used</span><span>{{ lb.total }}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="card" style="margin-top:16px;max-width:600px">
            ...
            <div *ngIf="leaveBalance.length>0"
              ...
              <div style="display:flex;gap:16px;flex-wrap:wrap">
                <span *ngFor="let lb of leaveBalance" style="font-size:13px;color:#555">
                  <strong style="color:#4a148c">{{ lb.leaveType }}</strong>: {{ lb.remaining }} days left
                </span>
              </div>
            </div>
            ...
            <form [formGroup]="form" (ngSubmit)="applyLeave()">
              <mat-form-field appearance="outline" style="width:100%">
                ...
                <mat-select formControlName="leaveTypeId" (selectionChange)="calcDays()">
                  <mat-option *ngFor="let t of leaveTypes" [value]="t.id">
                    {{ t.name }}
                    <span style="color:#aaa;font-size:12px"> — {{ getRemainingForType(t.id, t.name) }} days available</span>
                  </mat-option>
                </mat-select>
              </mat-form-field>
        ...
        <!-- TAB 3: My Applications — proper table -->
              ...
              <table style="width:100%;border-collapse:collapse">
                ...
                <tbody>
                  <tr *ngFor="let l of applications; let i=index"
                    ...
                    <td style="padding:14px 16px;text-align:center">
                      <button *ngIf="l.status==='PENDING'" mat-stroked-button (click)="cancel(l.id)"
        ...
        <!-- TAB 4: Holidays — directory-style grid -->
          ...
          <div style="margin-top:16px">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px">
              <div *ngFor="let h of holidays" class="card"
                ...
                [style.border-left-color]="h.isOptional?'#f57f17':'#2e7d32'">
                <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
                  <div style="width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0"
                    [style.background]="h.isOptional?'#fff8e1':'#e8f5e9'">
                    <mat-icon [style.color]="h.isOptional?'#f57f17':'#2e7d32'" style="font-size:20px">celebration</mat-icon>
                  </div>
                  <span [style.background]="h.isOptional?'#fff8e1':'#e8f5e9'"
                    [style.color]="h.isOptional?'#f57f17':'#2e7d32'"
                ...
                <div style="font-size:12px;color:#666;margin-bottom:4px;display:flex;align-items:center;gap:4px">
                  ...
                  {{ h.holidayDate | date:'EEEE, dd MMMM yyyy' }}
                </div>
...
export class EmployeeLeavesComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Backend returns: { leaveType: string, total: number, used: number, remaining: number }
    this.employeeService.getLeaveBalance().subscribe({
      next: (d: any) => {
        const raw = d?.data || d;
        this.leaveBalance = Array.isArray(raw) ? raw : [];
      },
      error: () => {}
    });
    this.leaveService.getActiveTypes().subscribe({
      next: (d: any) => { this.leaveTypes = Array.isArray(d)?d:(d?.data||[]); },
      error: () => {}
    });
    this.employeeService.getHolidays().subscribe({
      next: (d: any) => { const raw = d?.data||d; this.holidays = Array.isArray(raw)?raw:[]; },
      error: () => {}
    });
  ...
  loadApplications(): void {
    ...
    this.employeeService.getLeaveApplications().subscribe({
      next: (d: any) => {
        const raw = d?.data || d;
        this.applications = Array.isArray(raw) ? raw : [];
        this.loading = false;
      },
  ...
  // Calculate days from date strings for table display
  ...
  // Match by name since backend returns leaveType as string, not object
  getRemainingForType(typeId: number, typeName: string): number {
    const lb = this.leaveBalance.find(b =>
      b.leaveType?.toLowerCase() === typeName?.toLowerCase() ||
      b.leaveType?.toLowerCase().includes(typeName?.toLowerCase()?.split(' ')[0])
    );
    return lb?.remaining ?? 0;
  }

  isBalanceSufficient(): boolean {
    if (this.calculatedDays === 0) return true;
    const typeId = this.form.get('leaveTypeId')?.value;
    if (!typeId) return true;
    const type = this.leaveTypes.find(t => t.id === typeId);
    if (!type) return true;
    return this.getRemainingForType(typeId, type.name) >= this.calculatedDays;
  }

  getUsedPct(lb: any): number {
    if (!lb.total || lb.total === 0) return 0;
    return Math.round((lb.used / lb.total) * 100);
  }

  applyLeave(): void {
    if (this.form.invalid) return;
    this.employeeService.applyLeave(this.form.value).subscribe({
      next: () => {
        this.form.reset();
        this.calculatedDays = 0;
        this.loadApplications();
        this.employeeService.getLeaveBalance().subscribe({
          next: (d: any) => { const raw=d?.data||d; this.leaveBalance=Array.isArray(raw)?raw:[]; }
        });
  ...
  cancel(id: number): void {
    this.employeeService.cancelLeave(id).subscribe({
      next: () => { this.loadApplications(); this.snackBar.open('Leave cancelled','Close',{duration:2000}); }
    });
  }
}