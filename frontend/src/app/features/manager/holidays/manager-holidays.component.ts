...
export class ManagerHolidaysComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.http.get<any>(`${environment.apiUrl}/api/employee/holidays`).subscribe({
      next: (d: any) => {
        this.holidays = Array.isArray(d) ? d : (d?.data || []);
        this.holidays.sort((a, b) => new Date(a.holidayDate).getTime() - new Date(b.holidayDate).getTime());
        this.loading = false;
      },
