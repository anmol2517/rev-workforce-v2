    ...
    <div class="page-container">
      ...
      <div *ngIf="profile && !loading" style="display:grid;grid-template-columns:300px 1fr;gap:24px">
        ...
        <div class="card" style="padding:24px;text-align:center">
          ...
          <span class="status-chip APPROVED" style="display:inline-block;margin-top:6px">{{ profile.active }}</span>
          <div style="margin-top:16px;text-align:left;border-top:1px solid #f0f0f0;padding-top:14px">
            ...
            <p *ngIf="profile.phoneNumber" style="display:flex;align-items:center;gap:8px;color:#555;font-size:13px;margin:8px 0">
              <mat-icon style="font-size:16px;width:16px;height:16px">phone</mat-icon>{{ profile.phoneNumber }}
            </p>
...
export class ManagerProfileComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.managerService.getProfile().subscribe({
      next: (d: any) => {
        this.profile = Array.isArray(d) ? d[0] : (d?.data || d);
        this.initForm();
        this.loading = false;
  ...
  downloadPdf(): void {
    ...
    const html = `<html><head><title>Manager Profile</title>
      <style>
        ...
        <span class="badge">MANAGER</span><span class="badge">${p.active}</span>
        <div class="grid">
          ...
          <div class="box"><div class="label">Phone</div><div class="value">${p.phoneNumber || 'N/A'}</div></div>
