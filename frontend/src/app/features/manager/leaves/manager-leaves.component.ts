      ...
      <div *ngFor="let l of leaves" class="card" style="margin-bottom:14px;padding:20px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px">
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
              ...
              <div style="font-size:12px;color:#888">{{ l.leaveTypeName || l.leaveType?.name }}</div>
            </div>
          </div>
        ...
        <div *ngIf="l.status === 'PENDING'" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">
          <mat-form-field appearance="outline" style="flex:1;min-width:200px;margin-bottom:-1.25em">
            ...
            <input matInput [(ngModel)]="comments[l.id]" placeholder="Add a comment..." />
          </mat-form-field>
          <button mat-flat-button (click)="approve(l)"
            [disabled]="processingId === l.id"
            style="background:#2e7d32;color:#fff;gap:6px">
            <mat-icon>check_circle</mat-icon>
            {{ processingId === l.id ? 'Processing...' : 'Approve' }}
          </button>
...
export class ManagerLeavesComponent implements OnInit {
  ...
  load(): void {
    this.loading = true;
    this.managerService.getLeaveApplications().subscribe({
      next: (d: any) => { this.leaves = Array.isArray(d) ? d : (d?.data || []); this.loading = false; },
      error: () => { this.loading = false; }
    });
  }

  approve(l: LeaveApplication): void {
    this.processingId = l.id;
    this.managerService.approveLeave(l.id, this.comments[l.id] || 'Approved').subscribe({
  ...
  reject(l: LeaveApplication): void {
    if (!this.comments[l.id]?.trim()) {
      this.snackBar.open('Please add a reason for rejection', 'Close', { duration: 2000 }); return;
    }
    this.processingId = l.id;
    this.managerService.rejectLeave(l.id, this.comments[l.id]).subscribe({
