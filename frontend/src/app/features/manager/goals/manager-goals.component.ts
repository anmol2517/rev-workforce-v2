    ...
    <div class="page-container">
      ...
      <div *ngIf="showCreateForm" class="card" style="padding:24px;margin-bottom:24px;border-left:4px solid #1b5e20">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          <mat-form-field appearance="outline" style="grid-column:span 2">
            ...
            <mat-select [(ngModel)]="newGoal.employeeId">
              <mat-option *ngFor="let m of teamMembers" [value]="m.id">
          ...
          <mat-form-field appearance="outline" style="grid-column:span 2">
            ...
            <textarea matInput [(ngModel)]="newGoal.goalDescription" rows="3"
        ...
        <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:16px">
          ...
          <button mat-flat-button (click)="createGoal()"
            [disabled]="!newGoal.employeeId || !newGoal.goalDescription.trim() || !newGoal.deadline || creating"
            style="background:#1b5e20;color:#fff;display:flex;align-items:center;gap:6px">
      ...
      <div *ngIf="activeGoals.length > 0">
        <h2 style="font-size:16px;font-weight:700;color:#1a1a2e;margin-bottom:14px;display:flex;align-items:center;gap:8px">
          ...
          Active Goals <span style="font-size:13px;color:#888;font-weight:400">({{ activeGoals.length }})</span>
        </h2>
        <div *ngFor="let g of activeGoals" class="card" style="margin-bottom:12px;padding:20px">
          <ng-container *ngTemplateOutlet="goalCard; context:{g:g}"></ng-container>
        </div>
      </div>

      <div *ngIf="!loading && activeGoals.length === 0" class="empty-state">
      ...
      <ng-template #goalCard let-g="g" let-history="history">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px">
          <div style="flex:1;min-width:200px">
            ...
            <p style="font-size:14px;color:#333;margin:0 0 10px;line-height:1.5">{{ g.goalDescription }}</p>

            <div *ngIf="g.progressPercentage !== undefined && g.progressPercentage !== null">
              <div style="display:flex;justify-content:space-between;margin-bottom:4px">
                <span style="font-size:11px;color:#888">Progress</span>
                <span style="font-size:11px;font-weight:600;color:#1b5e20">{{ g.progressPercentage }}%</span>
              </div>
              <div style="height:6px;background:#e0e0e0;border-radius:3px;overflow:hidden">
                <div [style.width]="g.progressPercentage + '%'"
                  [style.background]="g.progressPercentage >= 100 ? '#2e7d32' : '#1b5e20'"
        ...
        <!-- Existing Comment -->
        <div *ngIf="g.progressComment && commentId !== g.id"
          ...
          <span style="font-size:13px;color:#555">{{ g.progressComment }}</span>
        </div>

        <!-- Comment Input -->
        <div *ngIf="commentId === g.id" style="border-top:1px solid #f0f0f0;padding-top:12px;margin-top:12px">
          <div style="display:flex;gap:10px;align-items:flex-end">
            ...
            <button mat-flat-button (click)="saveComment(g.id)" [disabled]="saving"
        ...
        <div *ngIf="!history" style="margin-top:12px">
          <button mat-stroked-button (click)="openComment(g)" style="color:#1b5e20;border-color:#1b5e20;font-size:13px">
            <mat-icon>comment</mat-icon> {{ g.progressComment ? 'Update Comment' : 'Add Comment' }}
          </button>
        </div>
      </ng-template>
...
export class ManagerGoalsComponent implements OnInit {
  ...
  loadGoals(): void {
    ...
    this.managerService.getGoals().subscribe({
      next: (d: any) => {
        this.allGoals = Array.isArray(d) ? d : (d?.data || []);
        this.loading = false;
      },
  ...
  loadTeam(): void {
    this.managerService.getTeamMembers().subscribe({
      next: (d: any) => { this.teamMembers = Array.isArray(d) ? d : (d?.data || []); },
      error: () => {}
    });
  }
  ...
  createGoal(): void {
    if (!this.newGoal.employeeId || !this.newGoal.goalDescription.trim() || !this.newGoal.deadline) return;
    this.creating = true;
  ...
  resetForm(): void {
    ...
    this.newGoal = { employeeId: null, goalDescription: '', deadline: '', priority: 'MEDIUM' };
  }

  getSelectedEmployeeName(): string {
    const emp = this.teamMembers.find(m => m.id === this.newGoal.employeeId);
  ...
  openComment(g: Goal): void {
    this.commentId = g.id;
    this.commentText = g.progressComment || '';
  }
  ...
  getInitials(g: Goal): string {
    return g.employee ? `${g.employee.firstName.charAt(0)}${g.employee.lastName.charAt(0)}` : '?';
  }
  getEmpName(g: Goal): string {
    return g.employee ? `${g.employee.firstName} ${g.employee.lastName}` : 'Unknown';
  }
