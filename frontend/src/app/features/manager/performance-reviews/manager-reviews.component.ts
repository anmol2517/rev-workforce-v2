    ...
    <div class="page-container">
          ...
          <div *ngFor="let g of completedGoals"
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
              ...
              <div>
                ...
                <div style="font-size:10px;color:#66bb6a">✅ Completed {{ g.updatedAt | date:'dd MMM yyyy' }}</div>
              </div>
            </div>
            <p style="font-size:12px;color:#555;margin:0;line-height:1.5">{{ g.goalDescription }}</p>
      ...
      <ng-template #reviewCard let-r="r" let-history="history">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px;margin-bottom:14px">
          <div style="display:flex;align-items:center;gap:12px">
            ...
            <div>
              ...
              <div style="font-size:12px;color:#aaa">Submitted: {{ r.createdAt | date:'dd MMM yyyy' }}</div>
            </div>
          </div>
          <div style="display:flex;gap:8px;align-items:center">
            <span *ngIf="r.employeeSelfRating"
              style="background:#fff8e1;color:#f57f17;padding:4px 10px;border-radius:12px;font-size:12px;font-weight:600">
              ⭐ Self: {{ r.employeeSelfRating }}/5
            </span>
        ...
        <div *ngIf="r.managerFeedback"
          ...
          <div style="font-size:13px;color:#333">{{ r.managerFeedback.feedback }}</div>
          <div style="margin-top:6px;display:flex;gap:8px;align-items:center">
            <span style="font-size:11px;color:#888">Rating:</span>
            <span style="font-weight:700;color:#7b1fa2">{{ r.managerFeedback.managerRating }}/5 ⭐</span>
          </div>
        </div>

        <!-- Feedback Form -->
        <div *ngIf="feedbackId === r.id" style="border-top:1px solid #f0f0f0;padding-top:14px">
          <form [formGroup]="feedbackForm" (ngSubmit)="submitFeedback(r.id)">
        ...
        <div *ngIf="feedbackId !== r.id && !history" style="margin-top:10px">
...
export class ManagerReviewsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.managerService.getPerformanceReviews().subscribe({
      next: (d: any) => {
        this.allReviews = Array.isArray(d) ? d : (d?.data || []);
        this.loading = false;
      },
    ...
    this.managerService.getGoals().subscribe({
      next: (d: any) => {
        ...
        this.completedGoals = this.allGoals.filter(g => g.status === 'COMPLETED');
      },
      error: () => {}
    });
  }
  ...
  getEmpCompletedGoals(r: PerformanceReview): Goal[] {
    ...
    return this.completedGoals.filter(g => g.employee && g.employee.id === r.employee!.id);
  }

  openFeedback(r: PerformanceReview): void {
    this.feedbackId = r.id;
    this.feedbackForm = this.fb.group({
      feedback: [r.managerFeedback?.feedback || '', Validators.required],
      managerRating: [r.managerFeedback?.managerRating || 3, Validators.required]
    });
  }
  ...
  getInitials(r: PerformanceReview): string {
    return r.employee ? `${r.employee.firstName.charAt(0)}${r.employee.lastName.charAt(0)}` : '?';
  }
  getEmpName(r: PerformanceReview): string {
    return r.employee ? `${r.employee.firstName} ${r.employee.lastName}` : 'Unknown';
  }
