        ...
        <div *ngFor="let e of team" class="card" style="padding:20px;border-top:4px solid #2e7d32">
          <div style="display:flex;align-items:center;gap:14px;margin-bottom:16px">
            ...
            <div style="display:flex;align-items:center;gap:4px">
              <span class="status-chip APPROVED" style="font-size:11px">{{ e.active }}</span>
              <button mat-icon-button (click)="viewProfile(e.id)"
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
            ...
            <div style="padding:10px;background:#f8f9fa;border-radius:8px">
              ...
              <div style="font-size:12px;color:#555">{{ e.phoneNumber || 'N/A' }}</div>
            </div>
          ...
          <div style="margin-top:14px;padding-top:12px;border-top:1px solid #f0f0f0">
            <button mat-flat-button (click)="viewProfile(e.id)"
...
export class TeamComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.managerService.getTeamMembers().subscribe({
      next: (d: any) => {
        this.team = Array.isArray(d) ? d : (d?.data || []);
        this.loading = false;
      },
      error: () => {
        this.loading = false;
